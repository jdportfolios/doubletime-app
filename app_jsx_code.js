...
import { Play, Pause, RotateCcw, BellOff, ChevronDown } from 'lucide-react';

// --- Sound Utility (Web Audio API) ---
// Modified slightly to close contexts to prevent browser limits (Max 6 contexts)
const playAlarmSound = (pitch = 440, type = 'square') => {
  ...
  // Critical fix: Close the context after sound plays to prevent memory leaks/browser limits
  setTimeout(() => {
    if(ctx.state !== 'closed') ctx.close();
  }, 600);
};

// --- Default SVG Characters (Fallbacks) ---
...
const AlienCharacter = ({ state }) => {
  ...
  return (
    <svg viewBox="0 0 200 200" className={`w-full h-full transition-transform duration-300 ${isAlarm ? 'animate-spin-slow' : ''}`}>
      ...
      {/* Tentacles with Stirring Animation */}
